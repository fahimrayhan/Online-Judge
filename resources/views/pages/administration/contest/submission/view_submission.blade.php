           	...
           	<div class="body subBody" style="overflow-x: scroll;scrollbar-width: none;">
                ...
                <table width="100%" id="submission_table" class="table-custom submission-table">
                	...
                	<tr class="submissionTr">
                		...
                		<td><a callback="(new Modal()).close()" href="{{route('profile',['handle' => $submission->user->handle])}}">{{$users[$submission->user->id]->main_name}}</a></td>
                		<td>{{$problemKey}}. {{$submission->problem->name}}</td>
                	...
                	<tr class="submissionTr testcase-tr">
                		<td>
                            <div id="view_test_case_btn_area_{{$testCase->id}}" style="display: {{($testCase->verdict->id != 16 && $testCase->verdict->id >2) ? 'block':'none'}}">
                                ...
                                <button url="{{route('administration.contest.submissions.view.test_case_details',['contest_id' => request()->contest_id,'submission_id' => request()->submission_id,'test_case_id' => $testCase->id])}}" onclick="submission.viewTestCaseDetail(this)" serial='{{$serial}}'><span id="view_test_case_btn_{{$serial}}" class="fa fa-angle-double-down"></span></button>
                            </div>
                        </td>
                		<td>{{$key+1}}</td>
